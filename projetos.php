<!DOCTYPE html>
<html lang="pt-br">
<head>
	<meta charset="UTF-8">
	<title>Projetos | Desenvolvedor Web</title>

	<!--[if lt IE 9]>
    <script src="resources/js/html5shiv.js"></script>
	<![endif]-->

	<link href='https://fonts.googleapis.com/css?family=Lato:400,100,300,700' rel='stylesheet' type='text/css'>
	<link rel="stylesheet" href="resources/css/boot.css"/>
	<link rel="stylesheet" href="resources/css/style.css"/>

 <link rel="shortcut icon" href="resources/img/faviconr.png"/>
</head>
<body>
	<header class="container bg-black">
		<div class="content">
			<h1 class="fl-left fontzero" >
				<a title="Rafael" href="#index" class="radius">
					Rafael Barros
				</a>
			</h1>

			<ul class="main_nav fl-right">
				<li><a title="Principal" href="index.php">Principal</a></li>
				<li><a title="Sobre" href="sobre.php">Sobre</a></li>
				<li><a title="Projetos" href="#">Projetos</a></li>
				<li><a title="Contato" href="contato.php">Contato</a></li>
        <li><a title="Inglês" href="en-us/projects.php"><img src="resources/img/eua_32.png"></a></li>
			</ul>
			<div class="clear"></div>
		</div>
	</header>

	<!--CONTEUDO-->
  <article class="container">
		<div class="content">
			<h1 class="fontzero">Desenvolvedor Web</h1>
				<article class="box box-small al-right">
					<h2 class="fontzero">Em Construção!</h2>
            <img src="http://www.ecoetrix.com.br/construcao.png">
				</article>
			<div class="clear"></div>
	</article>
	<!--CONTEUDO-->
	<footer class="container bg-black ">
		<section class="content">
			<h1 class="fontzero">Copyright</h1>
				<p class="al-center ">&copy; 2015 Rafael Barros.</p>
			<div class="clear"></div>
		</section>
	</footer>
</body>
</html>
